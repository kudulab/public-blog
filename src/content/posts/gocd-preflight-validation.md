...
With the new API and gocd-cli it is now possible to get the same feedback before pushing changes. It takes less than a second, depending on hosting environment. In this post I would like to encourage config-repo GoCD users to adopt the routine of checking configuration early. Below is a walkthrough the setup and usage.

The supporting code is hosted on [github](https://github.com/tomzo/gocd-preflight-demo). It may be useful to fork it now.
...
If for some reasons you need to setup GoCD manually, you can do so with following steps.
...
Start GoCD with docker, using following command:
```
...
GoCD CLI has to be configured to know where is the GoCD endpoint and how to authenticate with it.

If you have started with `./tasks setup_gocd`, then configuration has been done inside the docker container already.

If you are working with your own server, make notice of the server url, it should end with `/go`, for example `https://go.mydomain.com/go`.
Regardless if you are using the [gocd-cli-dojo](https://github.com/gocd-contrib/docker-gocd-cli-dojo) docker image or have downloaded one of the binaries, configuration can be done with a yaml file.
...
OK
```
Running the same command with `--verbose` is also interesting, we can see what has actually happened:
```console
 ...
 * the GoCD agent having sufficent secrets to access the GOCD API
...
The `gocd-cli` is in early development stage, but sufficent to boost productivity already.
